...
class Payment extends MY_Controller
{
    ...
    public function pay()
    {
        ...
        $ppn = ($amount + $type->fee) * 11 / 100;
        // $gross_amount = 10000;
        $gross_amount = ($amount + $type->fee) + $ppn;

        $items = $this->input->post('id_cart', true);
    ...
    function virtual_account($gross_amount)
    {
        ...
        try {
            ...
            $act = $this->TransaksiModel->create_bill_santri($data, $items);
    ...
    function qrcode($gross_amount)
    {
        ...
        $params = [
            'external_id' =>  $order_id,
            'type' => 'DYNAMIC',
            'callback_url' => 'https://sis.mindotek.com/Callback/QrCode',
            'amount' =>  $gross_amount,
            'expiration_date' => $expire->format(DateTimeInterface::ISO8601)
        ];
